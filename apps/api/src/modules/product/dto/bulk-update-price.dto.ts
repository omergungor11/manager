import { ApiProperty, ApiPropertyOptional } from '@nestjs/swagger';
import { Type } from 'class-transformer';
import {
  ArrayMinSize,
  IsArray,
  IsNumber,
  IsOptional,
  IsUUID,
  Min,
  ValidateIf,
  ValidateNested,
} from 'class-validator';

export class BulkPriceItemDto {
  @ApiProperty({ example: 'uuid-of-product', format: 'uuid' })
  @IsUUID()
  productId!: string;

  @ApiPropertyOptional({ example: 150.0, description: 'Cost price (purchase price)' })
  @IsOptional()
  @IsNumber()
  @Min(0)
  @Type(() => Number)
  costPrice?: number;

  @ApiPropertyOptional({ example: 220.0, description: 'Sale price' })
  @IsOptional()
  @IsNumber()
  @Min(0)
  @Type(() => Number)
  salePrice?: number;

  @ValidateIf((o: BulkPriceItemDto) => o.costPrice === undefined && o.salePrice === undefined)
  @IsNumber({}, { message: 'At least one of costPrice or salePrice must be provided per item' })
  _atLeastOne?: never;
}

export class BulkUpdatePriceDto {
  @ApiProperty({ type: [BulkPriceItemDto], description: 'Array of product price updates' })
  @IsArray()
  @ArrayMinSize(1)
  @ValidateNested({ each: true })
  @Type(() => BulkPriceItemDto)
  items!: BulkPriceItemDto[];
}
